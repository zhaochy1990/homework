...
func TestTextbookLifecycle(t *testing.T) {
	...
	// 成员角色。
	if err := db.Create(&model.ClassMember{ClassID: class.ID, UserID: u.ID, Role: "admin"}).Error; err != nil {
		t.Fatal(err)
	}
	role, isMember, err := s.MemberRole(ctx, class.ID, u.ID)
	if err != nil || !isMember || role != "admin" {
		t.Fatalf("MemberRole = %q, %v, %v", role, isMember, err)
	}
	if _, isMember, err := s.MemberRole(ctx, class.ID, u.ID+1_000_000); err != nil || isMember {
		t.Fatalf("non-member MemberRole = %v, %v", isMember, err)
	}
}
